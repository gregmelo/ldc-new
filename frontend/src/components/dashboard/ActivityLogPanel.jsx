...
const ACTION_CONFIG = {
  ...
  status_change:{ label: 'Changement statut',  icon: '🔄',  color: 'var(--warning-text)' },
  delete:       { label: 'Suppression',        icon: '🗑️',  color: 'var(--danger)' },
  login:        { label: 'Connexion',          icon: '🔐',  color: 'var(--text2)' },
  login_failed: { label: 'Connexion échouée',  icon: '⚠️',  color: 'var(--danger)' },
}
    ...
    <div className="card">
      <div className="section-header">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span style={{ fontSize: 12, color: 'var(--text3)' }}>{total} entrées</span>
          <button className="btn btn--sm" onClick={() => loadLogs(offset)}>↺ Actualiser</button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.map(log => {
            const cfg = ACTION_CONFIG[log.action] || { label: log.action, icon: '•', color: 'var(--text2)' }
            return (
              <div
                key={log.id}
                style={{
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12, color: 'var(--text2)', marginTop: 2 }}>
                    ...
                    {log.entity_id && <span style={{ color: 'var(--text3)' }}> · #{log.entity_id}</span>}
                  </div>
                  {log.description && (
